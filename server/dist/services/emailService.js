...
const cardDeletionOtpTemplate = (otp) => {
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
exports.cardDeletionOtpTemplate = cardDeletionOtpTemplate;
const inviteMemberTemplate = (name, email, username, password, loginUrl, expertise = []) => {
...
exports.inviteMemberTemplate = inviteMemberTemplate;
const broadcastTemplate = (message) => {
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
const programSuspendedTemplate = (programName, reason) => {
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
exports.programSuspendedTemplate = programSuspendedTemplate;
const escapeEmailText = (s) => String(s)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
const programBannedTemplate = (programName, reason, expiryNote) => {
...
const isReopenToTriagingEmail = (o) => o.newStatus === 'Triaging' &&
    !!o.oldStatus &&
    String(o.oldStatus) !== 'Triaging' &&
    ['Triaged', 'Closed', 'Duplicate', 'Resolved', 'Spam', 'NA', 'Paid', 'Out-of-Scope'].includes(String(o.oldStatus));
const isDuplicateClosureEmail = (o) => o.newStatus === 'Duplicate' && !!o.canonicalReportId;
const ACTION_HEADLINES = {
    ...
    status_change: (o) => {
        if (isReopenToTriagingEmail(o))
            return `Report reopened for triage`;
        if (isDuplicateClosureEmail(o))
...
const getStatusColor = (status) => {
    ...
    if (s.includes('pending'))
        return '#f97316';
    if (['spam', 'duplicate', 'na', 'out-of-scope', 'closed'].includes(s))
        return '#ef4444';
    return '#a1a1aa';
};
const getRoleIntro = (opts) => {
...
const reportEmailTemplate = (opts) => {
...
exports.reportEmailTemplate = reportEmailTemplate;
// Keep backward-compat alias for any old call sites still using the old signature
const threadNotificationTemplate = (recipientName, senderName, actionType, reportTitle, content, link) => (0, exports.reportEmailTemplate)({
    recipientName,
...
exports.threadNotificationTemplate = threadNotificationTemplate;
const walletTopUpTemplate = (companyName, amount, newBalance) => {
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
exports.walletTopUpTemplate = walletTopUpTemplate;
const payoutSuccessTemplate = (name, pkrAmount, newBalance, methodDesc) => {
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
exports.payoutSuccessTemplate = payoutSuccessTemplate;
const adminProfileUpdateTemplate = (userName, section, changes) => {
    const rows = changes
        .map((c) => `
      <tr>
