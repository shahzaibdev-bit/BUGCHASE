...
dotenv_1.default.config({ path: '../../.env' }); // Adjust path if needed, usually .env is in server root
const seedUsers = async () => {
    try {
        ...
        if (!process.env.DATABASE_URL) {
            ...
            await mongoose_1.default.connect(process.env.DATABASE_URL.replace('<PASSWORD>', process.env.DATABASE_PASSWORD || ''));
        }
        ...
        for (const u of usersToCreate) {
            const existing = await User_1.default.findOne({ email: u.email });
            if (existing) {
